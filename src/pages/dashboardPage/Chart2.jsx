...
export default function Chart2() {
    ...
    useEffect(() => {
        if (usersArr) {
            const sortedUsers = [...usersArr].sort((a, b) => b.impressions - a.impressions);
            const top5Users = sortedUsers.slice(0, 5);
            
            const newData = top5Users.map((user, index) => ({
                id: index,
                value: user.impressions,
                label: `User ${String.fromCharCode(65 + index)}`, 
            }));
    ...
    return (
        <Stack direction="row" width="100%" textAlign="center" spacing={2}>
            <Box flexGrow={1}>
                <Typography variant="h2" margin={5}>Top 5 User Impressions</Typography>
                <Box paddingLeft={10} ref={chartContainerRef} display={'flex'} justifyContent={'center'}> 
                    <PieChart
                        series={[
                            {
                                data: seriesData,
